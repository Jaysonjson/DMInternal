...
[%collapsible]
=====
If a Panel has buttons, they must be defined inside the Panels Block class. Once they are defined, DalekMod will automaticaly calculate the hitboxes of your model, for that, the buttons must be inside a bone (folder) with the name it was defined with in the class. + 
[source,java]
----
    @Override
